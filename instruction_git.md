...
Изображения обычно не хранят в основном файле. Для них создают специальный файл <`.gitignore`> в той же папке в которой находится основной файл и изображение, в котором прописывают ссылку на те изображения , которые необходимо игнорировать в основном файле , или просто все изображения (`*.jpg`)
...
Слияние используется в Git, чтобы собрать воедино разветвленную историю. Чтобы выполнить `слияние` необходимо ввести команду:
```
...
`! Перед слиянием обязательно выполните команду < git status > или /и команду < git branch >`. Это позволит убедиться, что HEAD-> указывает на ветку, принимающую результаты слияния. При необходимости выполните команду git checkout < master >, чтобы переключиться на принимающую ветку.
***
...
`-d` - Опция является указанием для `--delete`, который удаляет ветку только в том случае, если она уже была полностью объединена в ее восходящей ветке.

`-D` - Опция является указанием для `--delete --force`, который удаляет ветку "независимо от ее объединенного статуса".
***
## Просмотр графической структуры всех веток с коммитами

Изучение истории коммитов — важная составляющая работы с репозиторием. Ввиду ветвления с этой историей не всегда просто разобраться. Обычно для этой цели используют различными визуальными оболочки, но не всегда есть такая возможность. Временами приходится пользоваться средствами консоли, а именно командой:
```
git log --graph
или 
git log --all --oneline --graph
```
Эти команды позволяют `увидеть дерево изменений с хешами, датами измененй, авторами изменений и коммитами( либо сокрашенную версию с укороченными хешами и коммитами)`.
***
